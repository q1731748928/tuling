...
//导入需要调试的代码
var jscodess = fs.readFileSync(`${run_path}/bw.js`)
...
// 只需要替换 run.html 和 run.js 即可  5代没问题
function rs(){
...
function pdd(){
    const codeTest=`${configCode}${log_code}${toolsCode}${envCode}${globadlThis}${globalInit}${userInit}${proxyObj};;${jscodess}${asyncCode};console.table(myloglist);debugger;${last_deal}`
    const script = new VMScript(codeTest, "./debugJS.js")
    const result = vm.run(script);
    console.log(result)
    fs.writeFileSync(`${run_path}/output.js`,codeTest)
