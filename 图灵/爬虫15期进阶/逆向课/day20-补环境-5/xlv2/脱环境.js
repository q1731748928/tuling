...
getProtoEnvCode = function getProtoEnvCode(proto, instanceObj){
    ...
    try{
        ...
        code += `return Object.throwError("${e.name}", "${e.message}")`;
    }
    code += `}\r\n`;
    // 保护原型
    code += `Object.safeProto(${protoName}, "${protoName}");\r\n`;
    // 设置原型链
    ...
    for(const key in Object.getOwnPropertyDescriptors(proto)){
        ...
        code += `Object.defineProperty(${protoName}, "${key}", ${descriptor});\r\n`;
    }
    // 设置原型对象的属性
    for(const key in Object.getOwnPropertyDescriptors(proto.prototype)){
        ...
        code += `Object.defineProperty(${protoName}.prototype, "${key}", ${descriptor});\r\n`;
    }
    console.log(code);
    copy(code);
    // return code;
...
getObjEnvCode = function getObjEnvCode(obj, objName, instanceObj){
    ...
    for(const key in Object.getOwnPropertyDescriptors(obj)){
        ...
        code += `Object.defineProperty(${objName}, "${key}", ${descriptor});\r\n`;
    }
    console.log(code);
    copy(code);
}
    ...
    if(descriptor.hasOwnProperty("value")){
        ...
        if(value instanceof Object){
            if(typeof value === "function"){
                //
                code += `value:function ${prop} (){return "function ${prop}(){}"}`;
            }else{
                // 需要关注
    ...
    if(descriptor.hasOwnProperty("get")){
        ...
        if(typeof get === "function"){
            ...
            if(defaultValue === undefined || defaultValue instanceof Object){
                code += `get:function ${prop} (){return ""},`;
            }else{
                if(typeof defaultValue === "string"){
                    code += `get:function ${prop} (){return ""},`;;
                }else if(typeof value === 'symbol'){
                    code += `get:function ${prop} (){return  ${defaultValue.toString()}}, `;
                }else{
                    code += `get:function ${prop} (){return  ${defaultValue}}, `;
                }
            }
        }else{
            code += `get:undefined, `
...
code2=getObjEnvCode(location,'location')
// code2=getObjEnvCode(Navigator.prototype,'navigator'
// copy(code2)
// if (code1){
// code3=code1+"\r\n"+code2
// copy(code3)
