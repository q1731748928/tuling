...
class UserAgentDownloaderMiddleware:
    ...
    def process_request(self, request, spider):
        print('自定义UA随机请求头中间件被执行...')

        user_agent = random.choice(self.USER_AGENTS_LIST)
        request.headers['User-Agent'] = user_agent
        print('设置好的request对象中的UserAgent', request.headers)
        # Called for each request that goes through the downloader
        # middleware.
        ...
        # - or raise IgnoreRequest: process_exception() methods of
        #   installed downloader middleware will be called
        return None  # 返回None会将处理好的request对象传递给下载器


# class ProxyDownloaderMiddleware:
#     def process_request(self, request, spider):
#         print('下载中间件 - request')
#         request.meta['proxy'] = 'http://127.0.1:1087'
#         return None
#
#     def process_response(self, request, response, spider):
